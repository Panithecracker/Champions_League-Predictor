...
The sampling was done in C++ but we ended up discovering another inconvenience that popped up with this approach: if you just perform the simulation, there are instances where a team may be left unmatched unless a drawing rule is violated. In other words, based on the current draw sequence, it is possible to choose a plausible opponent that will later on have taken up all the possible opponents for some other team, forcing an invalid faceoff. Therefore, the simulation must handle this issue and detect it and discard the sampled in such a case. This is precisely how we programmed our algorithm but later on, we discovered a more sophisticated method which uses perfect-matching finding in a graph and efficient maximum matching algoritms from Grapth theory to compute the exact unconditional and conditional probabilities as the draws occur.We learned about the full details by reading Julien Guyon's and Frederic Meunier notes:  
